...
function updateInflow(time){  // inflow profile
  qIn=(time<90) ? 3400/3600 :
    (time<120) ? 0/3600 :
    (time<150) ? 3600/3600 :
    (time<150) ? 0/3600 :
    (time<180) ? 3400/3600 :
...
function updateRampflow(time){  // ramp flow profile
  qOn=(time<180) ? 700./3600 : 0; 
...
if there are at least 3 slowed down vehicles on the mainroad.
 Otherwise, the traffic light is always green.

Your aim is to propose a programmatic ramp-metering function which 
...
function setRampMeteringLight(mainroadvehicles,time){
  ...
  for(var i=0; i<mainroadvehicles.length; i++){
    if(mainroadvehicles[i].isRegularVeh()){ // no obstacle, traffic light
      ...
      if((mainroadvehicles[i]=="truck") && (speed<5) ){nTruckSlow++;}
    }
  }
  var red=false;
  if(nCongested>=1){red=true;}
  else if((nSlow>=2)&&((Math.floor(time))%10>2)){red=true;}
  if(nTruckSlow>0){red=true;}
...
var mergeLen=1.1*arcRadius;
var mainRampOffset=mainroadLen-0.6*straightLen+mergeLen-rampLen;
...
trafficObjs.activate(rampMeterLight,ramp,rampLen-mergeLen-distTL2merge);
...
//#################################################################
  ...
  var newTLstate=setRampMeteringLight(mainroad.veh,time);
  trafficObjs.setTrafficLight(rampMeterLight, newTLstate); 
