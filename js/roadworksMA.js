...
// has a speed below some critical speed
// action at the end of updateSim: stop the simulation if
// detectJam(mainroad.veh)=true.
// restart it with the restart button to keep settings unchanged

function detectJam(veh){
  var jamDetected=false;
  var speedCrit=20./3.6;
  for(var i=0; (i<veh.length)&&(!jamDetected); i++){
    if(veh[i].isRegularVeh()){
      jamDetected=(veh[i].speed<speedCrit);
    }
  }
  return jamDetected;
}
...
mainroadLen
  ...
  // MA: stop the simulation once a jam is detected

  var jamDetected=detectJam(mainroad.veh);
  if(jamDetected){myStartStopFunction();}
